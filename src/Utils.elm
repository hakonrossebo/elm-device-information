...
getLocationInfo location =
    case location of
        Just loc ->
            let
                altitude = Maybe.withDefault {value=0, accuracy=0} loc.altitude
                lat = roundTo 2 <| if (abs loc.latitude > 90) then 0 else loc.latitude
                lon = roundTo 2 <| if (abs loc.longitude > 180) then 0 else loc.longitude
                alt = roundTo 2 <| if (altitude.value > 10000) then 0 else altitude.value
                altAccuracy = roundTo 2 <| if (altitude.accuracy > 1000) then 0 else altitude.accuracy
            in
